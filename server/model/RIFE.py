...
class Model:
    ...
    def load_model(self, path, rank=0):
        ...
        #     self.flownet.load_state_dict(convert(torch.load('{}/flownet_epoch1.pkl'.format(path))))
        
        pkl_path = f'{path}/flownet.pkl'
        if os.path.exists(pkl_path):
            print("Loading model from:", pkl_path)
            self.flownet.load_state_dict(torch.load(pkl_path, map_location ='cpu'))
        else:
            print("No checkpoint found — training from scratch.")
        
    def save_model(self, path, rank=0):
        other_path = 'train_log/flownet.pkl'
        # print("inside basic rife saving")
        # print("rank: ", rank)
        # print("path: ", path)
        # print("other path: ", other_path)
        # print("Saving regular flownet")
        torch.save(self.flownet.state_dict(), other_path, _use_new_zipfile_serialization=False)
