...
def train(model, local_rank):
    ...
    for epoch in range(args.epoch):
        ...
        for i, data in enumerate(train_data):
            ...
            if step % 200 == 1:
                print("adding scalars to writer")
            ...
            if step % 1000 == 1:
                ...
                num_samples = min(len(merged_img), len(pred), len(gt))
        ...
        # I am updating this save model call to save a separate pkl file for each epoch
        # model.save_model(log_path, local_rank)  
        
        print("checking if folder exists")
        os.makedirs("train_log", exist_ok=True)  # Ensure train_log/ exists

        print("about to Save")
        model.save_model(f"train_log/flownet_epoch{epoch}.pkl", local_rank)
